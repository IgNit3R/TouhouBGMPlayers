...
### 3.4 2026-09-23 · 主窗口波形行（build 52）

| 号 | 内容 |
| --- | --- |
| `build-52` | **主窗口新增「整轨波形 + 乐评占位」行** —— 调研「路线 B」落地；独立扫描源 / 峰值分桶 / 滚轮缩放 / 播放头中位跟随；自检 21 → 33；四个坑的复盘 |
...
| `th06` `th06nc` `th07` `th075` `th08` `th09` `th095` `th10` `th105` `th11` `th12` `th123` `th125` `th128` `th13` `th135` `th14` `th143` `th145` `th15` `th155` `th16` `th165` `th17` `th175` `th18` `th185` `th19` `th20` | 29 份作品资料（`.md`）。⚠️ `th123` / `th145` / `th155` / `th175` 四份是「音乐室**信息**」（本作无评论，乐评字段承载署名/演奏信息） |
...
- 构建日志编号连续递增（`build` → `build-52`，其中 `build-8b` 是插入的补丁轮）。
...
- ⚠️ **写 `tools/` 脚本时的路径语义**（build-28 重排后定的，别再按老记忆写）：
  脚本里 **`parents[1]` = `bgmplayer/`、`parents[2]` = `thworks/`** —— 旧的 `parents[3]` 写法**全线作废**。
