...
### 3.5 2026-09-24 · 乐评区正文（build 53）

| 号 | 内容 |
| --- | --- |
| `build-53` | **乐评区从占位升级为正文显示** —— 只读可复制可滚动直角；默认日语、多语言口子（将来进设置）；内嵌 musiccmt.json.gz；自检 33 → 37 |
...
- 构建日志编号连续递增（`build` → `build-53`，其中 `build-8b` 是插入的补丁轮）。
