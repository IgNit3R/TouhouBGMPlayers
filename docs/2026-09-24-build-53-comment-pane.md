...
## 修订 4（2026-09-24 22:5x）：未播放时波形拉满整行 + 去掉「无波形」提示

用户反馈（启动界面截图）：未播放时波形只占一半、右半边空着并显示「无波形」三字。

### 1. `CommentColumn` 的**初始宽度改为 0**
`MainWindow.xaml` 里 `CommentColumn` 原静态值是 `1*` ✗ —— 启动时什么都还没播、本来就没有评论，
列却占着右半边 ⇒ 波形只拿 50% ✗。
改成初始 `0`（与「无评论的曲子」同款：整列收起、波形拉满）✓；
有评论时由 `RefreshComment` 设回 `1*` ✓。
⚠️ **列宽现在有三处要同步**：XAML 的初始值（0）、`RefreshComment` 的显示值（1*）、收起值（0）。

### 2. 彻底去掉「无波形」提示
`WaveformPanel` 原在无数据时居中画「无波形」✗，用户要求**两种情况都不显示**：
① 未播放时；② **黄昏作部分作品**——曲子已经在放、波形还在后台扫的那几秒
（会让人误以为这首没波形 ✗）。
改法：无数据时**什么都不画**，只留底槽与中线 ✓；连带删掉 `EmptyHint` 常量、`_hint` 画刷与 `MakeText` ✓。
⚠️ 注意：`EmptyHint` 那三个字是 `TextFaint` 画刷的**唯一**使用者，删掉后该字段一并清理。
